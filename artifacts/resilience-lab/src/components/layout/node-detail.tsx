...
import { X, Server, Activity, AlertTriangle, CheckCircle, ShieldAlert, Zap, Wifi, Wrench, Clock } from "lucide-react";
...
const NODE_TYPES: Record<string, { type: string; role: string }> = {
  "node-01": { type: "CORE",     role: "Primary routing backbone. Handles inter-zone traffic distribution." },
  "node-02": { type: "CORE",     role: "Secondary routing backbone. Failover partner to Alpha-Prime." },
  "node-03": { type: "EDGE",     role: "External traffic ingress/egress. Faces public network boundary." },
  "node-04": { type: "HUB",      role: "Central aggregation point for Sector B services." },
  "node-05": { type: "NODE",     role: "General-purpose compute node for distributed workloads." },
  "node-06": { type: "RELAY",    role: "Traffic relay between core and edge infrastructure." },
  "node-07": { type: "GATEWAY",  role: "API gateway. Routes external requests to internal services." },
  "node-08": { type: "BRIDGE",   role: "Cross-zone bridge. Connects Sector A to Sector B." },
  "node-09": { type: "MESH",     role: "Service mesh controller. Manages inter-service communication." },
  "node-10": { type: "VAULT",    role: "Encrypted secrets store. Manages credentials and certificates." },
  "node-11": { type: "CACHE",    role: "Distributed cache layer. Reduces backend load by 40%." },
  "node-12": { type: "SHIELD",   role: "Security enforcement point. Handles WAF and DDoS mitigation." },
  "node-13": { type: "FIREWALL", role: "Network perimeter firewall. First line of defence." },
  "node-14": { type: "CLUSTER",  role: "Compute cluster orchestrator. Manages job scheduling." },
  "node-15": { type: "LINK",     role: "Peering link to upstream providers. Critical uptime dependency." },
  "node-16": { type: "SENTINEL", role: "Monitoring node. Houses telemetry collectors and alerting." },
};
...
function LatencyBar({ latency }: { latency: number }) {
  ...
  const color = latency < 100 ? "bg-green-500" : latency < 300 ? "bg-amber-500" : "bg-red-500";
  return (
    <div className="mt-1">
...
export function NodeDetail({ node, onClose }: NodeDetailProps) {
  ...
  const nodeMeta = NODE_TYPES[node.id] ?? { type: "NODE", role: "Infrastructure node." };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" />
      <div
        className="relative z-10 w-[420px] bg-card border border-border rounded-lg font-mono shadow-2xl"
      ...
      >
        ...
        <div className="px-5 pb-2">
          <div className="text-[10px] text-muted-foreground tracking-widest mb-2 font-bold">CURRENT METRICS</div>
          <MetricRow label="LATENCY" value={`${Math.round(node.latency)}ms`} danger={node.latency > 500} warn={node.latency > 100} />
          <div className="pb-2 border-b border-border/50"><LatencyBar latency={node.latency} /></div>
          ...
          {node.assignedAgent && (
            <MetricRow label="ASSIGNED AGENT" value={node.assignedAgent.toUpperCase()} />
          )}
        </div>
        ...
        <div className="mx-5 mb-4 p-3 bg-background/40 rounded border border-border/50">
          ...
          <div className="grid grid-cols-3 gap-2 text-[10px]">
            <div className="text-green-400">✓ Normal<br/><span className="text-muted-foreground">≤100ms / &lt;2% err</span></div>
            <div className="text-amber-400">⚠ Degraded<br/><span className="text-muted-foreground">≤500ms / &lt;10% err</span></div>
            <div className="text-red-400">✗ Critical<br/><span className="text-muted-foreground">&gt;500ms / &gt;10% err</span></div>
          </div>
        </div>
        ...
        <div className="px-5 pb-5 flex gap-3">
          {(node.status === "failing" || node.status === "degraded") && !node.assignedAgent && (
            <Button
              variant="destructive"
              size="sm"
              className="flex-1 font-mono tracking-widest text-xs font-bold"
              onClick={() => repairNode.mutate({ nodeId: node.id })}
              disabled={repairNode.isPending}
            >
              <Wrench className="w-3.5 h-3.5 mr-2" />
              {repairNode.isPending ? "DISPATCHING…" : "DISPATCH ENGINEER"}
            </Button>
          )}
          {node.status === "offline" && (
            <Button
              variant="outline"
              size="sm"
              className="flex-1 font-mono tracking-widest text-xs border-amber-400/50 text-amber-400 hover:bg-amber-400/10"
              onClick={() => repairNode.mutate({ nodeId: node.id })}
              disabled={repairNode.isPending}
            >
